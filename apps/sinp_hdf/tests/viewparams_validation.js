...
const VIEWPARAMS_VALIDATION = {
  ...
  /**
   * Validation CD_REF - IDs entiers séparés par des pipes
...
function validateVIEWPARAMS(viewparams) {
  ...
  if (cdRefMatch) {
    ...
    if (cdRefValues && cdRefValues.endsWith("|")) {
      errors.push(`CD_REF ne doit pas se terminer par un pipe: ${cdRefValues}`);
    }
  }
...
describe("VIEWPARAMS Validation", () => {
  ...
  test("Détecte CD_REF avec pipe final", () => {
